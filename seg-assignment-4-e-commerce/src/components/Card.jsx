      ...
      <div class={`card card-custom ${isOnSale ? "on-sale" : ""}`}>
        ...
        <div class={`card-body row `}>
          <div className="col-8">
            ...
            <h5>{isOnSale ? <><s>${parseInt(currentPrice)+100}</s> ${currentPrice}</> : <>${currentPrice}</>}</h5>
            {isOnSale ? <p className="sale-text text-center">ON SALE!</p>: ""}
          </div>
          ...
          {shouldShowAddToCart ? 
            <Button
              ...
              onClick={handleAddToCartClick}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="black" class="bi bi-cart-plus" viewBox="0 0 16 16">
